...
// You will need to implement add() and displayList() before you test this function.
	...
	void changeLibraryType(Room *room, int newTypeLib){
		room -> lib = (libraryType) newLibType;
	}
...
void displayList()
{
	...
	Room *newRoom = NULL;
	if(type == hayden){
		newRoom = new Haydon(name_input, no_input, true);
	}
	else 
	{
		newRoom = new Noble(name_input, no_input, true);
	}
...
void load(string fileName)
{
	...
	for(int i = 0; i < numRoom; ++i)
	{
		...
		getline(inFile,name);
		inFile >> no;
		inFile >> libraryTypeInt;
		inFile.ignore();
	Room* newRoom = NULL;
	if(type == hayden){
		newRoom = new Hayden(name,no,type);
	
	}
	else if{
		newRoom = new Noble(name,no,type);
	}
	if(list == NULL){
		list = newNode;
	}
