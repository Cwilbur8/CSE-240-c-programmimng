...
int addPatient(string name_input, int age_input, int DOByear_input, int ID_input)
{
	if(patientCount >= MAX_PATIENTS)
		return 0;
	s[patientCount].setName(name_input);
	s[patientCount].setAge(age_input);
	s[patientCount].setDOByear(DOByear_input);
	s[patientCount].setID(ID_input);
...
void sort()
{
	Patient temp;
		for(int i = 0; i < patientCount - 1; i++){
			for(int k = i + 1; k < patientCount; k++){
			if(s[i].getAge() < s[k].getAge){
				temp = s[i];
				s[i] = s[k];
				s[k] = temp;
			}
...
void oldestPatient()
{
	...
	{
		if(s[i].getName() == startingCharacter)
		{
			if (s[i].getAge() > age)
			{
				age = s[i].getAge();
	...
	{
		cout << "No patient found with the given starting character." << endl;
	}
	delete newPatient;
}
